...
const PaginationContainer = (props) => {
    ...
    if(props.rowsPerPage === '*'){
        ...
        const   pageButtonsAmount = Math.min(props.pageButtonNo, Math.ceil(props.rowAmount/ props.rowsPerPage)),
            ...
            lastItemNo = Math.min(Math.max(1, props.page)  * props.rowsPerPage - 1, props.rowAmount),
            pageButtons = [],
        ...
        if(pageOffset > 1) {
            pageButtons.push(
                <th className="page-arrow-button-cell" key={'page-left-arrow'} onClick={() => props.setPage(1)}>
        ...
        for(let i = pageOffset; i < pageButtonsAmount + pageOffset; i++)
            if(i <= lastPage)
                if(i === props.page)
                    ...
                    pageButtons.push(<th key={i} onClick={() => props.setPage(i)}><button>{i}</button></th>)

        //add right arrow if needed
        if(props.page < lastPage - halfAvailableButtons && props.pageButtonNo < lastPage)
            pageButtons.push(
                <th className="page-arrow-button-cell" key={'page-right-arrow'} onClick={() => props.setPage(lastPage)}>
